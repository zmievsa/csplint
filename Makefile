...
format:
	shopt -s globstar; \
	poetry run pyupgrade **/*.py --py310-plus --exit-zero-even-if-changed; \
	poetry run autoflake . --in-place --recursive --remove-all-unused-imports --ignore-init-module-imports --verbose; \
	poetry run isort .; \
	poetry run black .;

test:
	poetry run pytest -v --cov=csplint --cov-report=term-missing:skip-covered --cov-branch --cov-report=xml tests;
